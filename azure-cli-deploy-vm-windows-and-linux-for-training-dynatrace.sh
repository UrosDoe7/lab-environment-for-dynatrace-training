...
do
        ...
        az vm run-command invoke -g "$RESOURCE_GROUP" -n "$DOMAIN" --command-id RunShellScript --scripts "apt-get install shellinabox && sed -i 's/4200/443/g' /etc/default/shellinabox && sudo apt-get install -y docker-compose && cd /home/user"$X$i"/.ssh && echo '"$RSAPUB1" "$RSAPUB2" "$RSAPUB3"'>> authorized_keys";
