...
do
	...
	az vm run-command invoke -g "$RESOURCE_GROUP" -n "$DOMAIN" --command-id RunShellScript --scripts "cd /home && git clone https://github.com/JLLormeau/dynatracelab_kubernetesaks.git && cd dynatracelab_kubernetesaks && chmod +x deploy-aks-cluster-with-azure-voting-app.sh";
	###stop VM Linux
