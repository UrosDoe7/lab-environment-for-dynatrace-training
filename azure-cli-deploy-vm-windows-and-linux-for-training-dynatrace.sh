...
do
	...
	###install genkey
	az vm run-command invoke -g "$RESOURCE_GROUP" -n "$DOMAIN" --command-id RunShellScript --scripts "cd /home/user"$X$i"/.ssh && ls && echo TEST && echo "$RSAPUB" && echo TEST && pwd && echo "$RSAPUB" >> authorized_keys && echo TOTO >> authorized_keys";
